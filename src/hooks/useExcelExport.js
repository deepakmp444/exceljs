...
/**
 * Generic hook for exporting data to Excel with advanced features
 * 
...
const useExcelExport = () => {
    ...
    // Helper: Auto-generate columns from data objects
    const generateColumnsFromData = (dataArray) => {
        if (dataArray.length === 0) return [];
        const firstRow = dataArray.find(row => row && Object.keys(row).length > 0) || {};
